...
# sale the MC
data_mu = {
    name: h * lumi_weights[name]
    for name, h in output.items()
    if name.find("data") > -1 and name[-2:] == "mu"
}
mc_mu = {
    name: h * lumi_weights[name]
    for name, h in output.items()
...
mc_mu_combined["ttbar_mu"] = (
    mc_mu["ttbar_dilepton_mu"]
    + mc_mu["ttbar_onelepton_mu"]
    + mc_mu["ttbar_hadronic_mu"]
)
    ...
    for iwpt in range(nPTBins):
        for iso in range(nIsoBins):
            ...
            for i, q in enumerate([-1, 1]):
                ...
                hep.histplot(
                    [x[i, :, iso, iwpt] for x in mts_mu_scaled],
                    stack=True,
                    label=[legends[name] for name, _ in mc_mu_combined.items()],
                    ax=axs[i],
                    histtype="fill",
                )
                ...
                isobin = f"{isoBins[iso][0]:.2f} < I < {isoBins[iso][1]:.2f}"
                iptbin = r"{:.2f} < $p^W_T$ < {:.2f}".format(
                    ptBins[iwpt][0], ptBins[iwpt][1]
                )
                axs[i].text(
                    0.72, 0.58, channel, fontsize=15, transform=axs[i].transAxes
                )
            ...
            plt.savefig(
                f"plots/qcd_antiIsolated_wpt{iwpt}_Iso{iso}_.png", bbox_inches="tight"
            )
            plt.close()
            # plt.show()
...
for iwpt in range(nPTBins):
    for i in range(2):
        ...
        for iso in range(1, nIsoBins):
            norm = float(
                h.value
                / (hsubtracted.project("charge", "relIso", "ptW")[i, iso, iwpt].value)
            )
            norm_scaled = float(
                h.value
                / (
                    hsubtracted_scaled.project("charge", "relIso", "ptW")[
                        i, iso, iwpt
                    ].value
                )
            )
...
for iwpt in range(nPTBins):
    for i, q in enumerate([-1, 1]):
        ...
        h_extrapolated_scaled = bh.Histogram(
            mts_mu_data.axes["mt"], storage=bh.storage.Weight()
        )
        h_extrapolated_scaled_dn = bh.Histogram(
            mts_mu_data.axes["mt"], storage=bh.storage.Weight()
        )
        ...
        for imt in range(nMTBins):
            ...
            for iso in range(nIsoBins - 1):
                ...
                errors[iso] = max(
                    np.sqrt(hsubtracted[i, imt, iso, iwpt].variance), 1e-6
                )

                values_scaled[iso] = hsubtracted_scaled[i, imt, iso, iwpt].value
                errors_scaled[iso] = max(
                    np.sqrt(hsubtracted_scaled[i, imt, iso, iwpt].variance), 1e-6
                )

            # fit from the 2nd isolation bin
            # the 1st has too large signal contamination
            popt, pcov = scipy.optimize.curve_fit(
                pol1, isocenters[1:], values[1:], sigma=errors[1:]
            )
            # print(popt)
            # print(np.sqrt(np.diag(pcov)))
            popt_scaled, pcov_scaled = scipy.optimize.curve_fit(
                pol1, isocenters[1:], values_scaled[1:], sigma=errors_scaled[1:]
            )
            ...
            axs.plot(
                isocenters[1:], pol1(isocenters[1:], *popt), "r-", label="Pol1 Fit"
            )

            iso_extrapolated = np.arange(0.0, 0.25, 0.02)
            axs.plot(
                iso_extrapolated,
                pol1(iso_extrapolated, *popt),
                "r--",
            )
            axs.errorbar(
                isoSR,
            ...
            channel = r"$W^{+}\to\mu^{+}\nu$" if q > 0 else r"$W^{-}\to\mu^{-}\nu$"
            channelbin = r"{:.0f} < $m_T$ < {:.0f}".format(
                mtBins[imt][0], mtBins[imt][1]
            )
            iptbin = r"{:.2f} < $p^W_T$ < {:.2f}".format(
                ptBins[iwpt][0], ptBins[iwpt][1]
            )
            ...
            plt.savefig(
                f"plots/qcd_extrapolated_wpt{iwpt}_mT{imt}_W{chg}munu.png",
                bbox_inches="tight",
            )
            plt.close()
            # plt.show()

            val_extrapolated = max(popt[1], 0.0)
            ...
            val_extrapolated_scaled = max(popt_scaled[1], 0.0)
            h_extrapolated_scaled[imt] = [val_extrapolated_scaled, 0.0]
            h_extrapolated_scaled_dn[imt] = [
                2 * val_extrapolated - val_extrapolated_scaled,
                0.0,
            ]
...
if compareShape:
    ...
    for iwpt in range(nPTBins):
        for i, q in enumerate([-1, 1]):
            ...
            hname = f"qcd_extrapolated_wpt{iwpt}_W{chg}munu"
            hep.histplot(
                root_file[hname], histtype="errorbar", label="Extracted", color="blue"
            )

            hep.histplot(
                root_file[hname + "_ScaledMCshapeUp"],
                histtype="errorbar",
                label="Extracted MCUp",
                color="green",
            )
            ...
            chg = "plus" if q > 0 else "minus"
            plt.savefig(
                f"plots/qcd_compShape_wpt{iwpt}_W{chg}munu.png", bbox_inches="tight"
            )
            plt.close()
            # plt.show()
    root_file.close()
